...
// Resource: Plural (AdminWidgets)
// Path: Plural (/admin/widgets)
// View Template Folder: Plural (/templates/admin/widgets/)

// AdminWidgetsResource is the resource for the Widget model
type AdminWidgetsResource struct {
	buffalo.Resource
}

// List gets all Widgets. This function is mapped to the path
// GET /admin/widgets
func (v AdminWidgetsResource) List(c buffalo.Context) error {
	...
	c.Set("template_prefix", "admin/widgets")
	return c.Render(200, r.Auto(c, widgets))
}

// Show gets the data for one Widget. This function is mapped to
// the path GET /admin/widgets/{admin_widget_id}
func (v AdminWidgetsResource) Show(c buffalo.Context) error {
	// Get the DB connection from the context
	tx, ok := c.Value("tx").(*pop.Connection)
	if !ok {
		return errors.WithStack(errors.New("no transaction found"))
	...
	// To find the Widget the parameter admin_widget_id is used.
	if err := tx.Find(widget, c.Param("admin_widget_id")); err != nil {
...
// This function is mapped to the path GET /admin/widgets/new
func (v AdminWidgetsResource) New(c buffalo.Context) error {
	// hint for Auto renderer to search proper template location
	c.Set("template_prefix", "admin/widgets")
...
// path POST /admin/widgets
func (v AdminWidgetsResource) Create(c buffalo.Context) error {
	// Allocate an empty Widget
	widget := &models.Widget{}
	...
	// and redirect to the admin/widgets index page
	return c.Render(201, r.Auto(c, widget))
}

// Edit renders a edit form for a Widget. This function is
// mapped to the path GET /admin/widgets/{admin_widget_id}/edit
func (v AdminWidgetsResource) Edit(c buffalo.Context) error {
	// Get the DB connection from the context
	tx, ok := c.Value("tx").(*pop.Connection)
	if !ok {
		return errors.WithStack(errors.New("no transaction found"))
...
// the path PUT /admin/widgets/{admin_widget_id}
func (v AdminWidgetsResource) Update(c buffalo.Context) error {
	// Get the DB connection from the context
	tx, ok := c.Value("tx").(*pop.Connection)
	if !ok {
		return errors.WithStack(errors.New("no transaction found"))
...
// to the path DELETE /admin/widgets/{admin_widget_id}
func (v AdminWidgetsResource) Destroy(c buffalo.Context) error {
	// Get the DB connection from the context
	tx, ok := c.Value("tx").(*pop.Connection)
	if !ok {
		return errors.WithStack(errors.New("no transaction found"))
	...
	// Redirect to the admin/widgets index page
	return c.Render(200, r.Auto(c, widget))
}
